...
export default function ChatPanel({ sessionId }: Props) {
  ...
  const send = async (text: string) => {
    ...
    try {
      const resp = await sendChatMessage(sessionId, text, messages);
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {SUGGESTED_QUESTIONS.map((q) => (
                <button
                  key={q}
                  onClick={() => send(q)}
                  className="text-xs bg-gray-800 text-blue-400 border border-gray-700 rounded-full px-3 py-1.5 hover:bg-gray-700 transition-colors touch-manipulation"
                >
                  {q}
                </button>
              ))}
      ...
      <div className="p-3 border-t border-gray-700/50">
        ...
        >
          ...
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white rounded-xl px-4 py-3 transition-colors active:scale-95 touch-manipulation"
          >
